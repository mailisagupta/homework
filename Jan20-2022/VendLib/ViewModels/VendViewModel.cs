...
namespace VendLib
{
    public class ViewModel : INotifyPropertyChanged
    {
        ...
        public ICanRack _canRack { get; private set; }
        public ICoinBox<Coin> _CoinBox1 { get; private set; }//temp
        public  ICoinBox<Coin> _CoinBox2 { get; private set; }//main
        ...
        public  DelegateCommand<string> EjectCanCommand { get; }
        public  DelegateCommand<decimal> AcceptCoinCommand { get;  }

       public  DelegateCommand<string> ReturnCoinCommand { get;  }

        public ViewModel(ICanRack canrack ,ICoinBox<Coin> tempCoinBox, ICoinBox<Coin> mainCoinBox
            )
        {
            _canRack = canrack;
            _CoinBox1 = tempCoinBox;
            _CoinBox2 = mainCoinBox;

            DisplayMessage = "Hello!! Click on Buttons below to pay";

            EjectCanCommand = new DelegateCommand<string>((f) => FromFlavorButton(f), (f) => CheckButtonIsEnabled(FlavorOps.ConvertStringToFlavor(f)));
            AcceptCoinCommand = new DelegateCommand<decimal>((d) => InsertCoin(d));
            ReturnCoinCommand = new DelegateCommand<string>(
                (c) => ReturnCoins());
        ...
        private void UpdateButtonStatus()
        {
            ...
            EjectCanCommand.RaiseCanExecuteChanged();
        }
